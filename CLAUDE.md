...
| `docs/superpowers/` | Claude Code workflow artifacts (design specs, plans). **Not vault content** — historical; do not read as current state. Graph-isolated: no links to/from real notes. |
...
- Superpowers artifacts (`docs/superpowers/`) are historical process artifacts, not durable vault content — never add links connecting them to or from real notes, and never reference them from MOCs or any other note. They must remain isolated from the knowledge graph.
